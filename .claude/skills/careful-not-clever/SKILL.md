...
else — which is what this section used to get wrong (UL-266): it read as a resource table
while listing eight `references/*.md` files this package does not ship.

**Correction, 2026-08-12 (UL-894).** An earlier revision of this paragraph said those eight
files *"never existed in any repo, in any history"*, and the paragraph below said no document
with that content *"was found in any household repo."* **Both readings were wrong about the
world.** All eight exist — ~52KB of them, alongside four more — at
`~/.hermes/skills/careful-not-clever/references/`, the deployed home of this very package.
They are **untracked**, so a search of git history returns nothing, honestly and misleadingly:
the measurement was right and the frame chose the answer. What follows is therefore a map of
where each topic lives, **not** a claim that anything is absent. Nothing here has been
established as nonexistent.
...
*Not in this table, and NOT absent:* remote-deploy cycle, spec→contract→test discovery,
abort/halt design language, and the 2026-06-27 verify-debug session note. Each has a real
document, currently outside version control:

| Topic | Document | Size |
|-------|----------|------|
| Remote deploy cycle | `~/.hermes/skills/careful-not-clever/references/cluster-deploy-workflow.md` | 2229 B |
| Spec → contract → tests | `~/.hermes/…/references/spec-contract-test-discovery.md` | 4215 B |
| Abort / halt design language | `~/.hermes/…/references/abort-design-language.md` | 4053 B |
| Verify-before-claim war story | `~/.hermes/…/references/session-2026-06-27-verify-debug.md` | 2665 B |

A dead pointer in a P0 skill is worse than an absent one — that reasoning was sound, and it is
why the citations came out. It does not license the sentence that came with them. **Getting
them back into version control is tracked as a P0** (`p0-layer-0-single-point-of-loss…`,
issue #2980) and is not a small job: `tools/skill-sync/skill-sync.py` declares
`FILE_NAMES = ("SKILL.md", "CAREFUL.md")`, so it structurally cannot carry a `references/`
directory, and committing the files here without fixing that would leave the pointers dead in
the sibling repos all over again.

**If you are an agent who just read "it doesn't exist" somewhere:** run
`node admin/skill-resource-check.mjs skills` and read the **UNTRACKED** class before you
delete anything. It searches the deployed skill homes as well as the package, precisely because
a git-framed search cannot tell "absent" from "unversioned" — and this household deleted eight
real documents' citations on that confusion.
